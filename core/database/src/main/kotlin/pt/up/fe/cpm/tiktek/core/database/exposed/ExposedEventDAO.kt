...
class ExposedEventDAO @Inject constructor(
    private val db: ExposedDatabaseConnection
) : EventDAO {
    override suspend fun getAll(): List<Event> = db.query {
        Events.selectAll().map { it.toEvent() }
    }

    override suspend fun getById(id: String): Event? = db.query {
        Events.selectAll().where { Events.id eq id }.map { it.toEvent() }.firstOrNull()
    }

    override suspend fun create(event: Event) = db.query {
        Events.insert {
            it.fromEvent(event)
        }
        event
    ...
    override suspend fun update(event: Event) = db.query {
    ...
    override suspend fun delete(id: String) = db.query {
...
private fun UpdateBuilder<Any>.fromEvent(event: Event) = apply {
